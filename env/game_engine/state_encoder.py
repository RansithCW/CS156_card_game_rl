...
def encode_state(game, player_id: int) -> np.ndarray:
    ...
    # --- 1. Hand and played cards encoding (32) ---
    hand_vec = game.current_hand_masks[player_id]
    played_vec = game.played_cards_mask
    ...
    # --- 3. Point vectors (Direct access)
    our_pts = game.team_points[player_id % 2]
    opp_pts = game.team_points[1 - (player_id % 2)]
    points_vec = np.array([
        our_pts / 304.0,
        opp_pts / 304.0,
        (our_pts - opp_pts) / 304.0
    ], dtype=np.float32)
    
    # --- 4. Highest Card in trcik (32) ---
